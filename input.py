...
def fix_data(pred_list,ans_list):
    ...
    for i,pred in enumerate(pred_s):
        ...
        while len(pred) != 0 :
            pred_charlist.append(pred[0:25])
            del title[0:25]

        while len(ryaku) != 0 :
            ans_charlist.append(ans[0:25])
            del ans[0:25]

        for pred_char in pred_charlist:
            sim_list = []
            list_a =[]
            for ans_char in ans_charlist:
                list_a.append(pred_char)
                #sim_list.append(cosine_similarity(title_char, ryaku_char))
            sim_list = cosine_similarity(list_a, ans_charlist)
            sim_list = sim_list[0]
            sim_list = sim_list.tolist()
            sim_index = sim_list.index(min(sim_list))
            fix_title = fix_title + ans_charlist[sim_index]
