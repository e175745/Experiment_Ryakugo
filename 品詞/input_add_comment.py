...
def conv_kana_to_vec(str_list,weight, TorR):
    ...
    for title in str_list:
        title_vec = []
        for char in title:
            vowel_list = [0]*5#母音は５種類
            consonant_list = [0]*20#子音20種類？
            #母音と子音の重きの振り分け
            #とりま、ん、ー、っ、ゔは「kana_lists」にないのでここで重さを決定
            if char == u"ン":
            ...
            """
            vow.append(vowel_list)
            con.append(consonant_list)
            vow = np.array(vow)
            con = np.array(con)
            title_vec.append(vow * con.T)
            """
        #足りない部分(要素？)の追加？
        if TorR=="T":
